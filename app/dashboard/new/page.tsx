...
export default function NewTripPage() {
  ...
  // Wrapped generatePackingList in useMemo. Previously, this expensive O(n) array generation
  // and subsequent string mapping was executing on every single keystroke in the form inputs
  // (name, destination, country) causing severe UI lag during typing.
  ...
  // ⚡ Bolt Performance Optimization:
  // Memoized the O(n) reduction calculation to prevent redundant processing on unrelated form state changes.
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-[1600px] mx-auto px-4">
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          {/* ── Form ── */}
          <form onSubmit={handleSubmit} className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-200 p-8 space-y-6">
            {error && (
            ...
            {/* Template toggle */}
            <div
              onClick={() => setFormData({ ...formData, generateSuggestions: !formData.generateSuggestions })}
            ...
            >
              ...
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                formData.generateSuggestions ? 'border-blue-500 bg-blue-500' : 'border-gray-300'
              }`}>
                {formData.generateSuggestions && (
            ...
            >
              {loading ? 'Creating Trip…' : formData.generateSuggestions ? 'Create Trip & Generate List' : 'Create Trip'}
            </button>
          </form>
              ...
              {formData.generateSuggestions ? (
                <>
                  ...
                  <p className="text-[11px] text-gray-400 mt-4 pt-3 border-t border-gray-100">
                    You can add, remove, or edit any item after creating the trip.
                  </p>
                </>
              ) : (
