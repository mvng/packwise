...
## 2026-04-16 - Improve TaskCard hover state accessibility
**Learning:** React state-driven hover actions (using `onMouseEnter` and `onMouseLeave`) make it impossible to use pure CSS pseudo-selectors (like `focus-within`) for keyboard accessibility, hiding important action buttons from keyboard-only users.
**Action:** When conditionally showing action buttons on hover, use Tailwind's `group`, `group-hover:opacity-100`, and `focus-within:opacity-100` instead of React state to ensure elements become visible when a user tabs into the component.
