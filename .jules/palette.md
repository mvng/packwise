...
## 2024-05-25 - Improve keyboard navigation of hover tooltips in TripMembersSection
**Learning:** Tooltips or nested interactive elements (like remove icons) relying purely on `group-hover` create invisible boundaries for keyboard users, as tabbed focus (`focus` or `focus-visible`) does not inherently trigger the hover state.
**Action:** When creating grouped hover states (e.g. `group-hover:opacity-100` or `group-hover:block`), pair them with `group-focus-within` equivalents to ensure the revealed elements appear during keyboard navigation.
