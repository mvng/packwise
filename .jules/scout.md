...
## 2024-05-04 - [Sitemap static routes anti-pattern]
**Learning:** When generating a `sitemap.ts` in Next.js, assigning `lastModified: new Date()` to static routes is an SEO anti-pattern because it falsely signals to crawlers that the page has changed on every build/request, causing crawl budget waste and potentially penalizing indexation efficiency.
**Action:** Always omit `lastModified` for static pages or hardcode the exact true modification date.
