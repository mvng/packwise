              ...
              {editingNotes?.id === item.id ? (
                <div className="mt-1 flex items-start gap-1" onClick={e => e.preventDefault()}>
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <button onClick={() => setEditingNotes(null)} aria-label={`Cancel editing note for ${item.name}`} className="p-1 bg-gray-100 text-gray-500 rounded hover:bg-gray-200"><X className="w-3 h-3" /></button>
                  </div>
                </div>
              ) : item.notes ? (
                <p
